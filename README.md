# node-dde-with-edge-js

node-dde-with-edge-js is a simplified regacy win32 Dynamic Data Exchange (DDE) wrapper for node.js using [Edge.js](https://github.com/tjanczuk/edge) and [NDde](http://ndde.codeplex.com/). The client listen to the asynchronous _advise_ on multi-threaded CLR without blocking the node.js event loop.
...
```
npm install node-dde-with-edge-js
```
...
```javascript
var dde = require("node-dde-with-edge-js");
...
var dde = require("node-dde-with-edge-js").createClient("myapp", "mytopic");
...
server.on("advise", function (topic, item, format) {
  console.log(
    "OnAdvise: " + "Topic: " + topic + ", Item: " + item + ", Format: " + format
  );
});
...
clients.on("disconnected", function (
  service,
  topic,
  isDisposed,
  isServerInitiated
...
server.onBeforeConnect = function (topic) {
  return true;
};
